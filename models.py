...
class Task(db.Model):
    ...
    def __repr__(self):
        return '<id задачи: {}, описание: {}, id пользователя: {}>'.format(self.id, self.description, self.user_id)
...
class User(db.Model, UserMixin):
    ...
    def __repr__(self):
        return '<id пользователя: {}, логин: {}, пароль: {}>'.format(self.id, self.login, self.pas)
    ...
    def save_to_db(self):
        db.session.add(self)
        db.session.commit()
        return 'Пользователь успешно добавлен ' + self.login
